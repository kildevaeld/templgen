...
class Expression {
    ...
    toJSON(human = false, location = false) {
            ...
            if (val instanceof Expression) {
                ...
                val = val.map(m => {
                    if (m instanceof Expression)
                        return m.toJSON(human, location);
                    return m;
                });
            }
            else if (key == "type") {
                val = types_1.Primitive[val];
            }
...
class ImportExpression extends Expression {
    constructor(location, path) {
        super(location);
        this.path = path;
        this.nodeType = types_1.Token.Import;
    }
}
...
function createExpression(type, position, ...args) {
    switch (type) {
        ...
        case types_1.Token.Import: return new ImportExpression(position, args[0]);
    }
    console.log(args);
