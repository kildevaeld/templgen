...
class UserValueVisitor extends visitor_1.AbstractExpressionVisitor {
    ...
    visitLoopExpression(e) {
        ...
        if (!this.first) {
            ...
            if (!(type.nodeType == types_1.Token.Array || (type.nodeType == types_1.Token.Primitive))) {
            ...
            if (type instanceof expressions_1.ArrayTypeExpression) {
                e.value.resolvedAs = type.type;
            }
            else {
                e.value.resolvedAs = type;
            }
    ...
    visitFunctionCallExpression(e) {
        ...
        if (!this.first) {
            ...
            for (let i = 0, ii = e.parameters.length; i < ii; i++) {
                ...
                let pt1 = getInner(p1), // (p1 as any).resolvedAs || p1,
                pt2 = getInner(p2); //(p2 as any).resolvedAs || p2;
                    ...
                    case types_1.Token.Primitive:
                        {
                            if (pt1.type !== pt2.type) {
                                throw new SemanticError(e.position, `invalid  primitive type parameter at #${i}`);
                            }
                        }
                        break;
                    case types_1.Token.CustomType:
                        {
                            if (pt1.name !== pt2.name) {
                                throw new SemanticError(e.position, `invalid custom type paramter at #${i}`);
                            }
                        }
                        break;
                }
    ...
    visitAccessor(e) {
        ...
        if (!this.first) {
            ...
            var index = 0;
            if (type.nodeType == types_1.Token.Array && len > 1 && e.names[1].match(/\d+/)) {
                type = type.type;
                if (len == 2) {
            ...
            if (type.nodeType !== types_1.Token.UserType && len > 1) {
                ...
                while (++i < len) {
                    if (e.names[i].match(/\d+/)) {
                        ...
                        type = type.type;
                        if (type.nodeType == types_1.Token.UserType) {
                            this.visit(type);
                            ct = type.resolvedAs;
                        }
                    }
                    ...
                    if (type.nodeType == types_1.Token.CustomType)
                        ct = type;
                    else if (type.nodeType === types_1.Token.Array && i < len - 1 && e.names[i + 1].match(/\d+/)) {
                        type = type.type;
                        i++;
                        if (type.nodeType === types_1.Token.UserType)
                            ct = type;
